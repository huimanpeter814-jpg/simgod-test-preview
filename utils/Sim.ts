...
export class Sim {
    ...
    applyTraits() { 
        ...
        if (this.mbti.includes('N')) { this.skillModifiers.logic = 1.3; this.skillModifiers.creativity = 1.3; this.skillModifiers.music = 1.2; } else { this.skillModifiers.cooking = 1.3; this.skillModifiers.athletics = 1.3; this.skillModifiers.gardening = 1.3; }
        if (this.mbti.includes('F')) { this.socialModifier *= 1.3; this.skillModifiers.dancing = 1.2; } else { this.socialModifier *= 0.8; this.skillModifiers.logic *= 1.2; }
        if (this.mbti.includes('J')) { this.metabolism.hygiene = 0.8; this.metabolism.energy = 0.9; } else { this.metabolism.fun = 1.4; this.skillModifiers.creativity *= 1.1; }
        const el = this.zodiac.element;
        if (el === 'fire') { this.skillModifiers.athletics *= 1.2; this.metabolism.energy *= 0.9; this.metabolism.social *= 1.2; } else if (el === 'earth') { this.skillModifiers.gardening *= 1.2; this.skillModifiers.cooking *= 1.2; this.metabolism.hunger *= 0.8; this.metabolism.social *= 0.9; } else if (el === 'air') { this.skillModifiers.logic *= 1.1; this.skillModifiers.music *= 1.2; this.metabolism.social *= 1.4; } else if (el === 'water') { this.skillModifiers.creativity *= 1.3; this.skillModifiers.dancing *= 1.1; this.socialModifier *= 1.2; }
    ...
    applyMonthlyEffects(month: number, holiday?: { name: string, type: string }) {
        ...
        if (holiday.type === 'traditional') {
            if (this.mbti.includes('E') || Object.keys(this.relationships).length > 5) { this.addBuff(BUFFS.festive_joy); this.say("过年啦！热闹热闹！🧨", 'act'); } 
            else if (this.mbti.includes('I')) { this.addBuff(BUFFS.social_pressure); this.say("亲戚好多...我想静静...", 'bad'); } 
            else { this.addBuff(BUFFS.vacation_chill); }
        } else if (holiday.type === 'love') {
            const hasLover = Object.values(this.relationships).some((r: Relationship) => r.isLover);
            if (hasLover) { this.addBuff(BUFFS.sweet_date); this.say("这个月要好好陪TA ❤️", 'love'); } 
            else {
                if (this.faithfulness > 60 || this.age > 28) { this.addBuff(BUFFS.lonely); this.say("又是一个人过节...", 'bad'); } 
                else { this.addBuff(BUFFS.playful); this.say("单身万岁！🍺", 'act'); }
            }
